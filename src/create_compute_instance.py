...
def main():
    ...
    try:
        ...
        compute_instance = ml_client.compute.get(args.instance_name)
        print(f"Compute instance '{args.instance_name}' already exists. Skipping creation.")
    except Exception as ex:
        print(f"Compute instance '{args.instance_name}' not found. Creating new compute instance.")
        ...
        # Create a compute instance with user assigned managed identity
            ...
            managed_identity = ManagedIdentityConfiguration(
                resource_id=f"/subscriptions/{args.subscription_id}/resourcegroups/{args.resource_group}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{args.user_assigned_identity}"
            )
            ua_identity_config = IdentityConfiguration(
